...
shape while preserving the current Pitch PID, PID Yaw, and AI Yaw
LQR/ESO behavior.

## Design

`Solve()` calculates Pitch first, then selects exactly one Yaw algorithm from
`ai_yaw_active_`. The PID Yaw and AI Yaw blocks remain behavior-identical
to the current `SolvePidYaw()` and `SolveAiYaw()` implementations, including
...
two former helper definitions and calls, and preserve the AI/PID branch and
